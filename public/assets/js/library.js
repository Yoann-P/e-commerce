...
export const manageCartLink = async (event) => {
  ...
  let cart = await fetchData(requestUrl);
...
export const manageCompareLink = async (event) => {
  ...
  if (requestUrl.search("/compare/remove/") != -1) {
    ...
    if (product) {
      addFlashMessage(
        `Product (${product.name}) removed from compare list !`,
        "danger"
      );
    } else {
...
export const manageWishListLink = async (event) => {
  ...
  if (requestUrl.search("/wishlist/remove/") != -1) {
    ...
    if (product) {
      addFlashMessage(
        `Product (${product.name}) removed from wish list !`,
        "danger"
      );
    } else {
  ...
  if (tbody) {
    ...
    if (compare) {
      let imageContainer = document.querySelector(
        "table.compare_table tbody tr.pr_image"
      );
      imageContainer.innerHTML = "";
      let nameContainer = document.querySelector(
        "table.compare_table tbody tr.pr_title"
      );
      nameContainer.innerHTML = "";
      let priceContainer = document.querySelector(
        "table.compare_table tbody tr.pr_price"
      );
...
export const addCompareEventListener = () => {
  let links = document.querySelectorAll(
    ".add-to-compare, .compare_table .remove_compare_item"
  );
  //console.log({ links });
  links.forEach((link) => {
...
export const addWishListEventListenerToLink = () => {
  let links = document.querySelectorAll(
    ".add-to-wishlist, .wishlist_table .remove-to-wishlist"
  );
...
export const addCartEventListenerToLink = () => {
  ...
  let add_to_cart_links = document.querySelectorAll(
    "a.add-to-cart, a.item_remove,  a.btn-addtocart"
  );
...
export const displayCart = (cart = null) => {
  ...
  if (tbody) {
    ...
    if (cart_sub_total_amount) {
      cart_sub_total_amount.innerHTML = formatPrice(cart.sub_total / 100);
    }
    if (cart_shipping_total_amount) {
      cart_shipping_total_amount.innerHTML = formatPrice(
        cart.carrier.price / 100
      );
    }
    ...
    wishlist.forEach((product) => {
      let content = `
            ...
            <td class="product-add-to-cart">
                <a href="/cart/add/${
                  product.id
                }/1" class="btn btn-fill-out btn-addtocart">
    ...
    cart.items.forEach((item) => {
      ...
      cart_list.innerHTML += `
                ...
                <a href="/product/${product.slug}">
                    <img width="50" height="50" alt="cart_thumb1" src="/assets/images/products/${
                      product.imageUrls[0]
                    }">
                    ${product.name}
                </a>
